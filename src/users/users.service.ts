...
export class UsersService {
  ...
  async findByUsername(username: string) {
    const [rows] = await this.pool.execute<RowDataPacket[]>(
      'SELECT id, username, password, role, refreshToken, created_at FROM users WHERE username = ?',
      [username],
    );
    return rows[0];
  }
  ...
  async findById(id: number) {
    const [rows] = await this.pool.execute<RowDataPacket[]>(
      'SELECT id, username, role, refreshToken created_at FROM users WHERE id = ?',
      [id],
    );
    return rows[0];
  }
  ...
  async getAll() {
    const [rows] = await this.pool.execute<RowDataPacket[]>(
      'SELECT id, username, role, refreshToken created_at FROM users',
    );
    return rows;
  }
  ...
  async setRefreshToken(id: number, refreshToken: string | null) {
    await this.pool.execute<OkPacket>('UPDATE users SET refreshToken = ? WHERE id = ?', [
      refreshToken,
      id,
    ]);
  }
  ...
  async findByRefreshToken(refreshToken: string) {
    const [rows] = await this.pool.execute<RowDataPacket[]>(
      'SELECT id, username, role FROM users WHERE refreshToken = ?',
      [refreshToken],
    );
    return rows[0];
  }
